...
| Atmosphere | Done. 21 backgrounds + 6 text effects, all real on-device names known, state readback works, quiet auto-refresh. `TextEffect`'s lyric-text mechanism **confirmed via real BT capture (2026-07-07)**: MiniToo acts as an AVRCP Controller, driving standard `GetElementAttributes`/`RegisterNotification(TRACK_CHANGED)` against the phone — not the custom SPP_JSON protocol at all, no brick risk, no new opcode. Full decode in PROTOCOL.md. **macOS system APIs (`MPNowPlayingInfoCenter`) confirmed NOT possible (2026-07-07)**: bare, +silent audio, and real Apple Music playback all fail to reach the MiniToo — macOS's own Bluetooth Now-Playing-push only targets paired Apple accessories (AirPods), confirmed via `log stream`. **But our own raw AVCTP channel works** (2026-07-07): `IOBluetoothDevice.openL2CAPChannelSync` to PSM `0x17` succeeds once the Divoom audio profile is disconnected (same precondition as RFCOMM), and the MiniToo immediately sends a real `GetCapabilities` AVRCP request over it — confirmed via a live decoded packet, same protocol as the Android capture. Not yet built: answering that request and `RegisterNotification`/`GetElementAttributes` to actually push custom title text. Known tradeoff: only works with real Mac audio disconnected. See `docs/local/dev-notes.md`'s macOS-side idea section and PROTOCOL.md's `TextEffect`/lyric-text section. |
